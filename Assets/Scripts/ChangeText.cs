...
public class ChangeText : MonoBehaviour
{
    ...
    void Update()
    {
        // REAL CODE. Times will have to be adjusted according to experimenter's needs.
        /*
        if (m.breakTime == true)
        {
        ...
        */
        ...
        if (m.breakTimeBlack && !m.breakTime && m.samAnswered && (m.CurrentTrial == 5 || m.CurrentTrial == 10))
        {
            changingText.text = "You can now start the next Trial by pressing the space bar.";
        }
        /*else
        {
            changingText.text = "testest " + m.DarkTimer.ToString("#");
        }
        */
        
       
        if (m.CurrentTrial <= 10 && !m.breakTime && !m.breakTimeBlack && !m.shortBreak)
        {
            if (m.isDark == true)
            {
                changingText.text = "Please wait for the door to change colour.";
        ...
        if (m.CurrentTrial > 10 && m.CurrentTrial <= 20 && !m.breakTime && !m.breakTimeBlack && !m.shortBreak)
        {
            if (m.isDark == true)
            {
                changingText.text = "Please wait for the door to change colour.";
